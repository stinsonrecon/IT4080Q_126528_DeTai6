...
@section('back-end.contents')
    ...
    <div class="overflow-x-auto flex flex-col">
        ...
        <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" method="POST"
            action="{{ route('promotion.update', ['id' => $promotions->id]) }}">
            @csrf
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">
                    Tên quảng cáo &nbsp <span class="text-red-500 text-base">*</span>
                </label>
                <input @if (session()->has('name')) value="{{ session()->get('name') }}"  @else value="{{ $promotions->name }}" @endif
            ...
            <div class="mb-6">
                ...
                <div class="flex @error('limitTime') is-invalid @enderror">
                    <div class="flex items-center mb-2 mr-4">
                        <input @if ($promotions->limitTime == 1 || session()->has('name')) checked @endif type="radio" value="1" id="radio-example-1" name="limitTime"
                            class="h-4 w-4 text-gray-700 px-3 py-3 border rounded mr-2"
                            onclick="displayDetailTime()">
                        <label for="radio-example-1" class="text-gray-600">Có</label>
                    </div>
                    <div class="flex items-center mb-2">
                        <input @if(($promotions->limitTime)==0&& empty((session()->has('name')))) checked @endif type="radio" value="0" id="radio-example-2" name="limitTime"
                            class="h-4 w-4 text-gray-700 px-3 py-3 border rounded mr-2"
                            onclick="displayDetailTime()">
            ...
            <div id="time" class="">
                <div class="mb-6 flex">
                    ...
                    <div>
                        ...
                        <input value="{{ substr($promotions->startTime, 0, 10) }}" type="date" id="start" name="date_start"
                            min="2021-01-01">
                    </div>
                </div>
                <div class="mb-6 flex">
                    ...
                    <div>
                        ...
                        <input value="{{ substr($promotions->endTime, 0, 10) }}" type="date" id="start" name="date_end"
                            min="2021-01-01">
                    </div>
                </div>
            </div>
